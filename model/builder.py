...
class Model:
    ...
    def install(self):
        ...
        # electron
        Util.execute(f"cd {build_dir} && npm install electron-is-dev --save && npm install -D dotenv-cli electron electron-builder")
        packageJson = fs.read.json("build/package.json")
        packageJson["type"] = "module"
        packageJson["main"] = "public/index.js"
    ...
    def electron(self):
        ...
        fs.remove("build/public")

        fs.copy("build/src/assets", "build/dist/build/assets")
        fs.copy("build/electron", "build/public")
        appfiles = self._search("build/src/app")
        whitelist = []
        for appfile in appfiles:
            filename = appfile.split("/")[-1]
            if filename not in ["app.json", "api.js"]: continue
            bundlefile = os.path.join("build", "dist", "build", *appfile.split("/")[1:])
            bundlefolder = os.path.dirname(bundlefile)
            fs.makedirs(bundlefolder)
            fs.copy(appfile, bundlefile)

            if filename != "api.js": continue
            code = fs.read(bundlefile)
            # auto generate ipcMain.on
            app_id = appfile.split("/")[-2]
            fnlist = Annotator.api.find_functions(code)
            for fn in fnlist:
                whitelist.append(f'"api.{app_id}.{fn}"')
            indexfile = "build/public/index.js"
            indexjs = fs.read(indexfile)
            events = Annotator.api.events(fnlist, app_id)
            fs.write(indexfile, f'{indexjs}\n\n{events}')

            # auto generate export default
            apijscode = Annotator.api.functions(code, fnlist)
            fs.write(bundlefile, apijscode)

        # auto generate contextBridge white list
        preloadfile = "build/public/preload.mjs"
        preload = fs.read(preloadfile)
        preload = preload.replace('"@wiz.electron.channels"', ", ".join(whitelist))
        fs.write(preloadfile, preload)
    ...
    def _reconstruct(self):
        ...
        fs.write('build/wizbuild.cjs', Code.ESBUILD)
    ...
    def _build(self):
        ...
        for app in fs.ls("build/src/app"):
            if fs.exists(os.path.join("build/src/app", app, "app.json")):
                ...
                # apijs = fs.read(os.path.join("build/src/app", app_id, "api.js"))
        ...
        # build pug files
        targets = self._search("build/src", result=[], extension=".pug")
        if fs.exists("build/wizbuild.cjs") == False:
            fs.copy("build/wizbuild.js", "build/wizbuild.cjs")
        if len(targets) > 0:
            targets = " ".join(targets)
            build_base_path = fs.abspath()
            Util.execute(f"cd {build_base_path} && node build/wizbuild.cjs {targets}")
    ...
    def _angular(self):
        ...
        if fs.exists("build/wizbuild.cjs") == False:
            fs.copy("build/wizbuild.js", "build/wizbuild.cjs")
        Util.execute(f"cd {esbuildpath} && node wizbuild.cjs")
