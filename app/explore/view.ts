...
export class Component implements OnInit {
    ...
    public getMod(path: string) {
        if (path.startsWith('src/electron')) return 'electron';
    ...
    public icon(node: any, checkopen: boolean = true) {
        ...
        if (node.id == 'src/electron') return 'text-red fa-solid fa-atom';
    ...
    public appStart() {
        wiz.call("electron_start");
    }

    public async appKill() {
        let res = await this.service.alert.show({ title: 'Kill', message: 'Are you sure to kill Electron?', action: "Kill", action_class: "btn-danger" });
        if (!res) return;
