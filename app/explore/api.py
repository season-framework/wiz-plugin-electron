...
def tree():
    ...
    if path == '' or path == 'src':
        ...
        children = []
        children.append(dict(title='electron', id='src/electron', type='folder', root_id='src'))
    ...
    if path == 'src/electron':
        if fs.exists(path) == False:
            fs.makedirs(path)
        if len(fs.ls(path)) == 0:
            fs.write(f'{path}/.env', Code.DOTENV)
            fs.write(f'{path}/index.js', Code.ELECTRON)
            fs.write(f'{path}/paths.js', Code.PATHS)
            fs.write(f'{path}/preload.mjs', Code.PRELOAD)

    root = driveItem(path)
    root_dirs = [
        'src/electron', 'src/angular', 'src/app/page', 'src/app/component', 'src/app/layout', 
...
def build(segment):
    # "el:dev": "DATA_PATH='.wetest' WIDTH=960 HEIGHT=640 APP_TITLE='TEST WIZ ELECTRON' electron .",
    builder.build()
    wiz.response.status(200)
...
def electron_start():
    ...
    proc = execute(f"cd {buildpath} && npm run el:dev")
    wiz.response.status(200)
...
def electron_kill():
    ...
    if os_name == "Linux":
        ...
        util = wiz.ide.plugin.model("src/util")
        execute = util.execute
        rootname = fs.abspath(os.path.join("..", "..")).split("/")[-1]
        cmd = f"kill -9 $(ps aux | grep '{rootname}' | grep 'Electron \\.' | awk '{{ print $2 }}')"
        execute(cmd)
